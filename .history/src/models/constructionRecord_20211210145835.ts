...
const constructionRecord = {
  ...
  effects: {
    ...
    *page({payload, callback}, {call, put}) {
      ...
      if (response.success) {
        console.log('fwfwefwfwefefw',response.data);
        ...
        if(payload.hasMore){
          yield put({
            ...
            refreshState: response.data.current*response.data.size >=response.data.total ? RefreshState.EmptyData : RefreshState.Idle
            },
          });
        }
       else{
