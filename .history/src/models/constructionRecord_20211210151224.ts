...
const constructionRecord = {
  ...
  effects: {
    ...
    *page({payload, callback}, {call, put}) {
      ...
      if (response.success) {
        if(payload.hasMore){
          console.log('fwfwefewf====seghsegr=',response.data);
          ...
          yield put({
            ...
            refreshState: response.data.current*response.data.size >=response.data.total ? RefreshState.HeaderRefreshing : RefreshState.Idle
            },
          });
        }
       else{
