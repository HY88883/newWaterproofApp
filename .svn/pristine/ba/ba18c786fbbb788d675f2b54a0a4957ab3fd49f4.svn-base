...
const formdata=useRef(null);
      ...
      teamMemberDetail({userId:params.id}).then(r=>{
        if(r.success){    
         ...
         if(data.teamId)   setTeamId(data.teamId);
                ...
                files[4] = {
                    bankcardOppositionLink: data.bankCardBackPhotosAttach.name,
                    bankcardOppositionId: data.bankCardBackPhotosAttach.id
                  };
                  setFiles([...files]);
  ...
  async function  onFinisht(values){
    if (!files[3]) {
      ...
      const params = {
        id:formdata.current?formdata.current.id:'',
        userId:formdata.current?formdata.current.userId:'',
        teamId:formdata.current?formdata.current.teamId:'',
