...
const ExpenseApproval = props => {
  ...
  const {
    expenseClaim: {
        expenseClaimList: {pagination, list},
      refreshState,
    },
  } = useSelector(state => ({
    expenseClaim: state.expenseClaim,
  }));
  ...
  const handleViewDetail = id => {
    navigation.navigate('ExpenseApprovalDetail', {id});
  };
      ...
      <Touchable
        ...
        <View>
          <View style={{marginBottom: px2dp(10)}}>
            <Text style={styles.biao}>{item.teamName}的费用报销申请</Text>
          </View>
          <View style={{flexDirection: 'row', alignItems: 'center'}}>
            <Text style={[styles.leftText]}>总额：</Text>
